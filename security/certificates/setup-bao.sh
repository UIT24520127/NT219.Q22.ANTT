...
BAO_API_URL="http://openbao:8200"
BAO_TOKEN=${BAO_PRODUCTION_TOKEN:-"root-token"}
...
until docker exec -e VAULT_TOKEN="$BAO_TOKEN" $CONTAINER_NAME bao status > /dev/null 2>&1; do
  echo "..."
  sleep 2
done
...
if ! docker exec -e VAULT_TOKEN="$BAO_TOKEN" $CONTAINER_NAME bao secrets list | grep -q "transit/"; then
  echo "[KMS INIT] Kích hoạt Transit Secret Engine..."
  docker exec -e VAULT_TOKEN="$BAO_TOKEN" $CONTAINER_NAME bao secrets enable transit
else
...
docker exec -e VAULT_TOKEN="$BAO_TOKEN" $CONTAINER_NAME bao write -f transit/keys/$KMS_KEY_NAME
