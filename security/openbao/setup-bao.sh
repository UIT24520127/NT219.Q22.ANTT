...
until docker exec $CONTAINER_NAME bao status > /dev/null 2>&1; do
  echo "..."
  sleep 2
done
...
if ! docker exec $CONTAINER_NAME bao secrets list | grep -q "transit/"; then
  echo "[KMS INIT] Kích hoạt Transit Secret Engine..."
  docker exec $CONTAINER_NAME bao secrets enable transit
else
...
docker exec $CONTAINER_NAME bao write -f transit/keys/$KMS_KEY_NAME
