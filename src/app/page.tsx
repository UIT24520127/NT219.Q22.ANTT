...
export default function LoginPage() {
  ...
  const [isRegistering, setIsRegistering] = useState(false); // Trạng thái chuyển đổi Đăng nhập / Đăng ký
  
  // Các biến lưu thông tin form
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [email, setEmail] = useState(''); // Bắt buộc cho đăng ký
  const [confirmPassword, setConfirmPassword] = useState('');

  // 1. Hàm tạm cho nút Passkey
  const handleDummyPasskey = (e: React.MouseEvent) => {
    e.preventDefault();
    alert("Tính năng quét vân tay/Passkey sẽ được hoàn thiện sau!");
  };

  // 2. Logic gọi API Đăng nhập
  ...
  // 3. Logic xử lý Đăng ký (Giao diện gọi Backend)
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Gửi dữ liệu xuống Next.js Backend (cái API route vừa viết)
      ...
      if (response.ok) {
        ...
        setIsRegistering(false); // Lật form về lại bên Đăng nhập
        setPassword(''); // Xóa trắng mật khẩu cho an toàn
        setConfirmPassword('');
      } else {
          ...
          <>
            ...
            {loginMethod === 'passkey' ? (
              <div className="flex flex-col gap-4">
                <p className="text-sm text-gray-400 text-center mb-2">Đăng nhập nhanh chóng bằng sinh trắc học.</p>
                <button 
              ...
              <form className="flex flex-col gap-4" onSubmit={handlePasswordLogin}>
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-gray-900 border border-gray-600 rounded-lg p-3 pl-10 text-white focus:outline-none focus:border-blue-500"
                  />
          ...
          <form className="flex flex-col gap-4" onSubmit={handleRegister}>
            <div className="relative">
              <input 
                type="text" placeholder="Tên đăng nhập" required
                value={username} onChange={(e) => setUsername(e.target.value)}
                className="w-full bg-gray-900 border border-gray-600 rounded-lg p-3 pl-10 text-white focus:outline-none focus:border-green-500"
              />
              <User className="w-5 h-5 absolute left-3 top-3 text-gray-400" />
            </div>
            <div className="relative">
              <input 
                type="email" placeholder="Email bắt buộc" required
            ...
            <button 
              type="submit" disabled={loading}
              className="w-full bg-green-600 hover:bg-green-700 p-3 rounded-lg font-bold transition-all mt-2 disabled:opacity-50"
            >
